...
resource "aws_iam_role" "api_gateway_invoke_lambda" {
    ...
    assume_role_policy = "${file("./templates/assume-role-api-gateway.json")}"
}
...
resource "aws_iam_role_policy" "api_gateway_invoke_lambda" {
    ...
    policy = "${file("./templates/api-gateway-invoke-lambda-policy.json")}"
}
...
resource "aws_iam_role" "lambda_execution" {
    ...
    assume_role_policy = "${file("./templates/assume-role-lambda.json")}"
}

resource "aws_iam_role_policy" "lambda_execution_policy" {
    name = "${var.resource_prefix}-lambda-execution-policy"
    role = "${aws_iam_role.lambda_execution.id}"
    policy = "${file("./templates/lambda-execution-policy.json")}"
}
